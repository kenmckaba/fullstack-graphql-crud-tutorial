  ...
  return {
    [Symbol.asyncIterator]: () => {
      ...
      const listener = (data: any) => {
        if (listening) {
          listeners.forEach(resolve => resolve({ value: data }));
          listeners.length = 0;
        }
      };
...
const resolvers = {
  ...
  Mutation: {
    ...
    updateUser: (_parent: any, args: { id: string; name?: string; age?: number; isMarried?: boolean; }): User | null => {
      ...
      const updatedUser: User = {
        ...
        isMarried: args.isMarried !== undefined ? args.isMarried : existingUser.isMarried,
      };

      users[userIndex] = updatedUser;

      // Publish the updated user to subscribers
      console.log('Publishing USER_UPDATED event:', { userUpdated: updatedUser });
      eventEmitter.emit('USER_UPDATED', { userUpdated: updatedUser });

      return updatedUser;
    },
    deleteUser: (_parent: any, args: { id: string }): boolean => {
      const userIndex = users.findIndex(user => user.id === args.id);
